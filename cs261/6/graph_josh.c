...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	source->isVisited = 1;
	addFrontListDeque(&q, source);

	while(!isEmptyListDeque(&q))
	{
		fail = 0;

		if(frontListDeque(&q)->numNeighbors < 1)
			fail = 1;

		if(fail == 0)
		{
			visited = 0;
			for(i = 0; i < frontListDeque(&q)->numNeighbors; i++)
			{
				if(frontListDeque(&q)->neighbors[i]->isVisited == 1)
				{
					visited += 1;
				}
			}

			while(visited != frontListDeque(&q)->numNeighbors )
			{
				visited = 0;
				if(frontListDeque(&q)->neighbors[j]->isVisited == 1)
				{
					j += 1;
					addedToFront = 0;
				}
				...
				{
					frontListDeque(&q)->neighbors[j]->isVisited = 1;
					addFrontListDeque(&q, frontListDeque(&q)->neighbors[j]);
					j = 0;
					addedToFront = 1;
				}
		...
		if(frontListDeque(&q) == destination)
			return 1;
		else
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while(!isEmptyListDeque(&q))
	{
		if(EQ(frontListDeque(&q), destination))
			return 1;
		
		for(j = 0; j < frontListDeque(&q)->numNeighbors; j++)
		{		
			if(frontListDeque(&q)->neighbors[j]->isVisited == 0)
			{
				frontListDeque(&q)->neighbors[j]->isVisited = 1;
				addBackListDeque(&q, frontListDeque(&q)->neighbors[j]);
			}
		}
...
void computeAdjacencyMatrix(Graph* g)
{
	...
	int** adjMat = (int**) malloc(sizeof(int*) * g->numVertices);
	for(i = 0; i < g->numVertices; i++)
		adjMat[i] = (int*) calloc(g->numVertices, sizeof(int));

	for(i = 0; i < g->numVertices; i++)
	{
		for(j = 0; j < g->numVertices; j++)
		{
			for(k = 0; k < g->vertexSet[i].numNeighbors; k++)
			{
				if(g->vertexSet[i].neighbors[k] == &g->vertexSet[j])
				{
					adjMat[i][j] = 1;
					break;
				}
...
void computeReachabilityMatrix(Graph* g, int useDFS)
{
	...
	int** reaMat = (int**) malloc(sizeof(int*) * g->numVertices);
	for(i = 0; i < g->numVertices; i++)
		reaMat[i] = (int*) calloc(g->numVertices, sizeof(int));
	...
	for(i = 0; i < g->numVertices; i++)
		for(j = 0; j < g->numVertices; j++)
		{
			if(i == j)
				...
				reaMat[i][j] = DFS(g, &g->vertexSet[i], &g->vertexSet[j]);
			else
				reaMat[i][j] = BFS(g, &g->vertexSet[i], &g->vertexSet[j]);
		}

	_printMatrix(g, reaMat);
