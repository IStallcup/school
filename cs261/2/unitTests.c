...
void secondaryTest()
{
	...
	addDynArr(myAry, one);
	addDynArr(myAry, two);
	addDynArr(myAry, three);
	addDynArr(myAry, four);
	addDynArr(myAry, five);
	addDynArr(myAry, six);
	addDynArr(myAry, seven);
	addDynArr(myAry, eight);
	addDynArr(myAry, nine);
...
void testAddDynArrResizing(UnitTestResult* result)
{
	...
	else if(myAry->data[3] != fourthValToAdd)
		result->errMsg = "Fourth value not added";
	else
		result->success = 1;
}
...
{
	int capacity = 4;
	TYPE firstValToAdd = 23;
	TYPE secondValToAdd = 982;
	TYPE thirdValToAdd = 147;
	TYPE firstValToSeek = 23;
	TYPE secondValToSeek = 1;
	TYPE firstContainsSample;
	TYPE secondContainsSample;
	...
	firstContainsSample = containsDynArr(myAry, firstValToSeek);
	secondContainsSample = containsDynArr(myAry, secondValToSeek);
	
	if (myAry->size == 0)
		result->errMsg = "Size is zero.";
	else if(firstContainsSample == 0)
		result->errMsg = "Did not find first element.";
	else if(secondContainsSample != 0)
		result->errMsg = "Found member not of array.";
...
{
	int capacity = 5;
	TYPE firstValToAdd = 87;
	TYPE secondValToAdd = 2;
	TYPE thirdValToAdd = 201;
	TYPE fourthValToAdd = 493;
	TYPE fifthValToAdd = 1;
	TYPE firstValRm = 2;
	TYPE secondValRm = 3;
	TYPE removedFirstValRm;
	TYPE removedSecondValRm;
	TYPE containsFirstValRm;
	TYPE containsSecondValRm;
	...
	addDynArr(myAry, thirdValToAdd);
	addDynArr(myAry, fourthValToAdd);
	addDynArr(myAry, fifthValToAdd);
	
	containsFirstValRm = containsDynArr(myAry, firstValRm);
	containsSecondValRm = containsDynArr(myAry, secondValRm);
	removedFirstValRm = removeDynArr(
	
	if (containsFirstValRm == 0)
		result->errMsg = "Did not find first element.";
	if (containsSecondValRm == 1)
		result->errMsg = "Incorrectly found second element.";
	else
		result->success = 1;
