...
**	Authors: Josh Bowen, Isaac Stallcup
**
***********************************************************************/
...
int _heightOfSubTree(Node* root)
{
	...
	leftHeight = _heightOfSubTree(root->left) + 1;
	rightHeight = _heightOfSubTree(root->right) + 1;
...
Node* _addNodeToSubtree(Node* curr, TYPE val)
{
	...
	else if(val.priority == curr->val.priority)
		return curr;
	else if(val.priority > curr->val.priority)
...
int _containsSubTree(Node* curr, TYPE val)
{
	...
	if(curr == NULL)
		...
		temp = _containsSubTree(curr->right, val);
	else if(val.priority < curr->val.priority)
		temp = _containsSubTree(curr->left, val);
...
TYPE _removeLeftMost(Node* curr, Node* parent, Node* origAncestor)
{
	...
	if(curr->left == NULL)
	{
		...
		removeThisVar = _removeLeftMost(curr->left, curr, origAncestor);
