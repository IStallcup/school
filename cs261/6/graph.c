...
#include "ListDeque.h"
/**************************************************************
**	CS261 Summer 2015 Assignment 6
**	Josh Bowen, Isaac Stallcup
**************************************************************/
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while (!isEmptyListDeque(&q))
	{
		...
		{
			if (frontListDeque(&q)->neighbors[i]->isVisited == 0)
			{
				frontListDeque(&q)->neighbors[i]->isVisited = 1;
				addBackListDeque(&q, frontListDeque(&q)->neighbors[i]);
			}
		}
...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	while (!isEmptyListDeque(&stack))
	{
		...
		for (i = 0; i < current->numNeighbors; i++)
		{
			...
			if (EQ(temp, destination))
				return 1;
			else if (temp->isVisited != 1)
			{
				addFrontListDeque(&stack, temp);
				temp->isVisited = 1;
			}
		}
	}
...
void computeAdjacencyMatrix(Graph* g)
{
	/*initialize matrix */
	int i, j, k;
	int **adMatrix = (int**)malloc(sizeof(int*)*g->numVertices);
	for (i = 0; i < g->numVertices; i++)
		adMatrix[i] = (int*)calloc(g->numVertices, sizeof(int));
		
	for (i = 0; i < g->numVertices; i++)
		for (j = 0; j < g->numVertices; j++)
			for (k = 0; k < g->vertexSet[i].numNeighbors; k++)
				if (EQ(g->vertexSet[j].label, g->vertexSet[i].neighbors[k]->label))	
					adMatrix[i][j] = 1;
		
	/*print matrix */	
	_printMatrix(g, adMatrix);
...
{
	int i, j;
	int **reMatrix = (int**)malloc(sizeof(int*)*g->numVertices);
	for (i = 0; i < g->numVertices; i++)
		reMatrix[i] = (int*)calloc(g->numVertices, sizeof(int));
	...
	for (i = 0; i < g->numVertices; i++)
		...
		{	
			if (useDFS == 1)
				reMatrix[i][j] = DFS(g, &g->vertexSet[i], &g->vertexSet[j]);
			else reMatrix[i][j] = BFS(g, &g->vertexSet[i], &g->vertexSet[j]);
		}
	
	_printMatrix(g, reMatrix);
