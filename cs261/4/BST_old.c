...
Node* _createNode(TYPE val)
{
	...
	assert(created);
	created->val = val;
	created->val.priority = val.priority;
	created->left = NULL;
	created->right = NULL;
	return created;
}
...
int _heightOfSubTree(Node* root)
{
	...
	if (root == NULL)
		return 0;
	else if (root->right == NULL && root->left == NULL)/* statement not necessary, but here for ease of readability */
		return 1; 
	heightL = _heightOfSubTree(root->left) + 1;
	heightR = _heightOfSubTree(root->right) + 1;
	if (LT(heightL, heightR))
		return heightR;
	else return heightL;
}
...
Node* _addNodeToSubtree(Node* curr, TYPE val)
{
	if (curr == NULL)
	{
		curr = _createNode(val);
		return curr;
	}
	else if (curr->val.priority == val.priority) return curr;
	else if (curr->val.priority > val.priority)
	{
...
{
	if (EQ(curr->val.priority, val.priority))
		return 1;
	else if (EQ(_containsSubTree(curr->left, val), 1))
		return 1;
	else if (EQ(_containsSubTree(curr->right, val), 1))
		return 1;
	else return 0;
}
