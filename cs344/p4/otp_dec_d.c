...
//	printf("file to decode=%s\n",buffer);
	...
	if (SRC == NULL)
		error("ERROR otp_dec_d opening src");
	buffer = strtok(NULL," \n");
//	printf("key file=%s\n",buffer);
	FILE* KEY = fopen(buffer,"r");
	if (KEY == NULL)
		error("ERROR otp_dec_d opening key");
	tpos = 0;	
	do //while key is not empty and while file is not empty
	{
		...
		if (i == EOF || i == '\n')
		{
			//look at if the file is too short
...
int main(int argc, char* argv[])
{
	...
	if (argc < 2)
	{
		fprintf(stderr,"ERROR otp_dec_d, no port proivded\n");
		exit(1);
	}
	...
	//error checking
	if (sockfd < 0)
		error("ERROR otp_dec_d opening socket");
		//brewster's error code; does what expected? maybe
	
	if (setsockopt(sockfd,SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) < 0)
		error("ERROR otp_dec_d setting socket");
	...
	if (bind(sockfd, (struct sockaddr *) &serv_addr,
				sizeof(serv_addr)) < 0)
		error("ERROR otp_dec_d on binding");
	...
	while(1)
	{
		...
		//error checking
		if (newsockfd < 0)
			error("ERROR otp_dec_d on accept");
		switch(cPid = fork())
		{
			case 0:
			//	printf("in child\n");
				...
				if (n < 0)
					error("ERROR otp_dec_d reading from socket");
			...
			//	printf("cryptb = %s\n",cryptb);	
				...
				if (n < 0)
					error("ERROR otp_dec_d writing to socket");
				exit(0);
				break;
			default:
			//	printf("in parent\n");
